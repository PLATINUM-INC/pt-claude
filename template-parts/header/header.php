...
$auto_menu = get_field('auto_menu', 'option');
...
<header class="header js-header" id="header">
    ...
    <?php if ($auto_menu) : ?>
        <?php
        // Auto menu logic - build menu from pages and taxonomies
    ...
    <nav id="filters" class="filters-nav desktop-only">
        <div class="container">
                    ...
                    <?php foreach (array_slice($parents_with_children, 0, 4) as $parent) { ?>
                        ...
                        <li class="menu-item dropdown">
                            <button class="dropdown-toggle dropdown-toggle--js" type="button">
                                <span><?php echo esc_html(pt_menu_limit_words($title, 3)); ?></span>
                            </button>
            ...
            <?php else : ?>
                <!-- WordPress Menu from admin -->
                <?php
                wp_nav_menu([
                    'theme_location' => 'header-menu',
                    'container' => false,
                    'menu_class' => 'filters-nav-list',
                    'walker' => new PT_Desktop_Menu_Walker(),
                    'fallback_cb' => false,
                ]);
                ?>
            <?php endif; ?>
    ...
    <div class="mobile-filters-panel js-filters-panel">
        ...
        <?php else : ?>
            <!-- WordPress Menu from admin (mobile) -->
            <?php
            wp_nav_menu([
                'theme_location' => 'header-menu',
                'container' => false,
                'menu_class' => 'mobile-filters-list',
                'walker' => new PT_Mobile_Menu_Walker(),
                'fallback_cb' => false,
            ]);
            ?>
        <?php endif; ?>
